...
export function selectModelByComplexity(input: string): string {
  ...
  const hasLongSentences = input.split(/[.!?]/).some(sentence => sentence.length > 150);
  const hasTechnicalTerms = /\b(function|formula|VBA|SQL|database|query|macro|script|code|debug|error)\b/i.test(input);
